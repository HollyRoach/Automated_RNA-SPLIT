################################################################################
#SCRIPT INSCPECTS TOTAL MOLECULE COUNT DATA - gives quick insight into the data 
#please dontact Holly Roach at hmroach@hotmail.co.uk if you have any questions
################################################################################
#returns basic summary stats for total molecule count data
...
if (!dir.exists(paste(File_Path, Cell_Type, "Molecule_Count", New_Line_Name, sep="/"))) {
...
Reference_Line <- read_csv(paste(File_Path, Cell_Type, "Molecule_Count", "All_Cell_Lines", paste(Reference_Line_Name, "Total_Molecule_Count_Compile.csv", sep="_"), sep="/"))

New_Cell_Line <- read_csv(paste(File_Path, Cell_Type, "Molecule_Count", "All_Cell_Lines", paste(New_Line_Name, "Total_Molecule_Count_Compile.csv", sep="_"), sep="/"))
...
molecule_stats <- Reference_Line %>% 
  bind_rows(New_Cell_Line) %>% 
  group_by(Cell_Line, Data_Set, Phase) %>%                           #group data
  summarise(n=n(),                                         #find number of data points
            Mean=mean(Total_Centroids),                       #finds the mean 
            Median = median(Total_Centroids),                 #find the median
            Upper_Quartile = quantile(Total_Centroids, 0.75), #find the upper quartile
            Lower_Quartile = quantile(Total_Centroids, 0.25), #find the lower quartile
            SD=sd(Total_Centroids)) %>%                       #finds standard deviation
  mutate(SE = SD/sqrt(n),                                  #finds standard error of mean
         CI_95 = SE * qt((1-0.05)/2 + .5, n-1)) %>%        #finds 95% confidence limit
  ungroup() %>% 
  arrange(Phase)                                           #orders data
...
File_Name <-paste(New_Line_Name, "Total_Molecule_Count_Summary_Stats.csv", sep="_")

write_csv(molecule_stats, paste(Save_Path, File_Name, sep="/"))
