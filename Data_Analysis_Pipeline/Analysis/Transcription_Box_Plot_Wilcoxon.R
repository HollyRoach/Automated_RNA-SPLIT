...
for (phase in Phase_List) {
  for (time in Timepoints) {
    ...
    Stat_Diff <- wilcox.test(wilcox_data[which(wilcox_data$Cell_Line==Reference_Line_Name),]$No_Centroids,
                             wilcox_data[which(wilcox_data$Cell_Line==New_Line_Name),]$No_Centroids,
                             exact=FALSE) #as there is less than 50 data pointsset extact = FALSE to prevent to approximate p-value (see R documentation for more details)
    ...
    Diff_Less <- wilcox.test(wilcox_data[which(wilcox_data$Cell_Line==Reference_Line_Name),]$No_Centroids,
    ...
    Diff_More <- wilcox.test(wilcox_data[which(wilcox_data$Cell_Line==Reference_Line_Name),]$No_Centroids,
    ...
    p_values <- tibble(a, b, c) %>%
      transmute(Comparision = paste(phase, time, Reference_Line_Name , "vs", New_Line_Name, sep='_'),
                General_Stat_Diff = a,
...
#is there a difference between the 2 data sets?
#is there a general difference between the data sets?
Exp_Stat_Diff <- wilcox.test(Expansion_Only[which(Expansion_Only$Cell_Line==Reference_Line_Name),]$No_Centroids,
...
#if there is a difference, is it statistically less 
Exp_Diff_Less <- wilcox.test(Expansion_Only[which(Expansion_Only$Cell_Line==Reference_Line_Name),]$No_Centroids,
...
#if there is a difference, is it statistically more
Exp_Diff_More <- wilcox.test(Expansion_Only[which(Expansion_Only$Cell_Line==Reference_Line_Name),]$No_Centroids,
...
Exp_p_values <- tibble(e, f, g) %>%
  transmute(Comparision = paste("Overall_Initiation", Reference_Line_Name , "vs", New_Line_Name, sep='_'),
            General_Stat_Diff = e,
...
if (Exp_Stat_Diff$p.value < p) {
  if (Exp_Diff_Less$p.value < Exp_Diff_More$p.value ){
    outcome_1 <- paste("Overall, the new cell line has statistically higher trancription rate during expansion phase than the", Reference_Line_Name, sep=" ")
  } else {
    outcome_1 <- paste("Overall, the  new cell line has statistically lower transcription rate during expansion phase than the", Reference_Line_Name, sep=" ")
  }} else {
    outcome_1 <- paste("Overall, there is no statistical difference in transcription rate during expansion phase between the new cell line and the", Reference_Line_Name, sep=" ")
  }
...
#is there a difference between the 2 data sets?
#is there a general difference between the data sets?
SS_Stat_Diff <- wilcox.test(Steady_State_Only[which(Steady_State_Only$Cell_Line==Reference_Line_Name),]$No_Centroids,
...
#if there is a difference, is it statistically less 
SS_Diff_Less <- wilcox.test(Steady_State_Only[which(Steady_State_Only$Cell_Line==Reference_Line_Name),]$No_Centroids,
...
#if there is a difference, is it statistically more
SS_Diff_More <- wilcox.test(Steady_State_Only[which(Steady_State_Only$Cell_Line==Reference_Line_Name),]$No_Centroids,
...
SS_p_values <- tibble(h, i, j) %>%
  transmute(Comparision = paste("Overall_Maintenance", Reference_Line_Name , "vs", New_Line_Name, sep='_'),
            General_Stat_Diff = h,
...
if (SS_Stat_Diff$p.value < p) {
  if (SS_Diff_Less$p.value < SS_Diff_More$p.value ){
    outcome_2 <- paste("Overall, the new cell line has statistically higher trancription rate during steady-state phase than the", Reference_Line_Name, sep=" ")
  } else {
    outcome_2 <- paste("Overall, the new cell line has statistically lower transcription rate during steady-state phase than the", Reference_Line_Name, sep=" ")
  }} else {
    outcome_2 <- paste("Overall, there is no statistical difference in transcription rate during steady-state phase between the new cell line and the", Reference_Line_Name, sep=" ")
  }
