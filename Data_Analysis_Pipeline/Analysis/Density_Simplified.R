...
Name_2 <- bquote("Methylation_Removed") #sets name for new cell line
Name_1 <- bquote("Normal_Cell")                   #sets name for reference_line
...
Title <- "Density of Xist RNA within the Nucleus"
x_axis <- "Cell_Line"
y_axis <- "Distance between Xist RNA [nm]"
...
Present_Data <- All_Density_Data %>%
  filter(Cell_Line %in%
           c(Refernece_Line, New_Line_Name, Cell_Line_4)) #select which cell lines to present
