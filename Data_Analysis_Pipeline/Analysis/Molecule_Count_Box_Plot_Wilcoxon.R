...
Stat_Diff <- wilcox.test(All_Molecule_Count_Data[which(All_Molecule_Count_Data$Cell_Line==Reference_Line_Name),]$Total_Centroids,
...
Diff_Less <- wilcox.test(All_Molecule_Count_Data[which(All_Molecule_Count_Data$Cell_Line==Reference_Line_Name),]$Total_Centroids,
...
Diff_More <- wilcox.test(All_Molecule_Count_Data[which(All_Molecule_Count_Data$Cell_Line==Reference_Line_Name),]$Total_Centroids,
...
if (Stat_Diff$p.value < p) {
  if (Diff_Less$p.value < Diff_More$p.value){
    outcome_1 <- paste("Overall, the new cell line (", New_Line_Name,") has statistically higher total centroid count than the", Reference_Line_Name, sep=" ")
  } else {
    outcome_1 <- paste("Overall, the new cell line (", New_Line_Name,") has statistically lower total centroid count than the", Reference_Line_Name, sep=" ")
  }} else {
