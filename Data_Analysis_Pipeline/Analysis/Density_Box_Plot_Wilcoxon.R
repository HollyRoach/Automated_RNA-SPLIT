...
if (Stat_Diff$p.value < p) {
  if (Diff_Less$p.value < Diff_More$p.value){
    outcome_1 <- paste("The new cell line (", New_Line_Name,") has statistically greater distances between Xist RNPs (lower density) than the", Refernece_Line, sep=" ")
  } else {
    outcome_1 <- paste("The new cell line (", New_Line_Name,") has statistically shorter distances between Xist RNPs (higher density) than the", Refernece_Line, sep=" ")
  }} else {
    outcome_1 <- paste("There is no statistical difference in Xist RNP distances (density) between the new cell line (", New_Line_Name,") and the", Refernece_Line, sep=" ")
  }
