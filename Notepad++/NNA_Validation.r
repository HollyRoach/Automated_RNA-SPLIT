...
################################################################################
#STEP 2:NNA - plot the data

Present_Data <- WT_Manual_Line %>%
  bind_rows(WT_Auto_Line) %>%               
  mutate(Cell_Line = case_when(Cell_Line == "WT_manual" ~ "WT_manual",
                               Cell_Line == "WT_auto" ~ "WT_auto")) %>%
  mutate(Phase = case_when(Phase == "EdU" ~ "EdU",                                  
                           Phase == "Initiation" ~ "Expansion",
                           Phase == "Maintenance" ~ "Steady_State",
                           Phase == "random_sample" ~ "Random",)) %>%
  mutate(Key = paste(Phase, Cell_Line, sep="_"))  

#order data to be present in violin plot
Present_Data$Key <- factor(Present_Data$Key, 
                           levels = c("EdU_WT_manual", "Expansion_WT_manual", "Steady_State_WT_manual", "Random_WT_manual",
                                      "EdU_WT_auto", "Expansion_WT_auto", "Steady_State_WT_auto", "Random_WT_auto"), ordered = TRUE) 
...
theme <- theme(plot.title = element_text(family = "Calibri (Body)", face = "bold", size = (13)),
               ...
               legend.text = element_text(family = "Calibri (Body)", size = (11)), 
               axis.title = element_text(family = "Calibri (Body)", face = "bold", size = (11)),
               axis.text = element_text(family = "Calibri (Body)", face = "bold", size = (9)))
...
NNA_plot <- ggplot(Present_Data, aes(x = Phase, y = Distance, fill = Key)) +          #assigns data to be plotted, sets colour based on cell line
  ...
  facet_wrap(~Cell_Line, nrow = 1) +                                                  #separates different Cell Lines into separate plots
  coord_cartesian(ylim=c(0, 990)) +                                                  #adjusts scale of the y axis 
  scale_fill_manual(values = c("#FEE0D2", "#FC9272", "#EF3B2C", "#FCBBA1",            #WT_manual Colours
                               "#F0F0F0", "#BDBDBD", "#737373", "#D9D9D9")) +         #WT_auto Exp Colours
  geom_boxplot(width=0.1, outlier.shape = NA, aes(fill = Key)) +                      #adds box plot over the violin plots
  labs(title = "D) Validate Nearest Neighbour Analysis",                                     #sets name of the axes/title
       x = "",
       y = "Distance to Nearest Neighbour [nm]") + 
  theme                                                                               #adds the theme which defines the font, text size etc
...
#IS THERE A STATISTICAL DIFFERENCE BETWEEN THE Expansion phases?
t <- filter(Present_Data, Phase == "Expansion")

#is there a general difference in distances recorded between the data sets?
Exp_Stat_Diff <- t.test(t[which(t$Cell_Line == "WT_manual"),]$Distance,
                                    t[which(t$Cell_Line == "WT_auto"),]$Distance)
...
Exp_Diff_Less <- t.test(t[which(t$Cell_Line == "WT_manual"),]$Distance,
                                    t[which(t$Cell_Line == "WT_auto"),]$Distance, alternative = "less")
Exp_Diff_Less$p.value #p = 1 suggests new cell line lower distances than Random 

#if there is a difference in distance, is it statistically Greater?
Exp_Diff_Greater <- t.test(t[which(t$Cell_Line == "WT_manual"),]$Distance,
                                       t[which(t$Cell_Line == "WT_auto"),]$Distance, alternative = "greater")
Exp_Diff_Greater$p.value #p = 1 suggests new cell line has higher distances than Random

#record  Random p values in table
Exp_Diff <- Exp_Stat_Diff$p.value
Exp_Less <- Exp_Diff_Less$p.value
Exp_Greater <- Exp_Diff_Greater$p.value

Exp_p_values <- tibble(Exp_Diff, Exp_Less, Exp_Greater) %>%
  transmute(Comaprision = "Expansion_Data",
            General_Stat_Diff = Exp_Diff,
            Diff_Statistically_Less = Exp_Less,
            Diff_Statistically_Greater = Exp_Greater) 

#says whether there is a stat difference between the new expansion data and Random Control
if (Exp_Stat_Diff$p.value < 0.05) {
  if (Exp_Diff_Less$p.value < Exp_Diff_Greater$p.value){
    outcome_1 <- "The WT_auto has statistically higher distances in Expansion phase than the WT_manual"
  }  else {
    outcome_1 <- "The WT_auto has statistically lower distances in Expansion phase than the WT_manual"
  }}else {
    outcome_1 <- "There is no statistical difference in distances between the WT_auto in Expansion phase and the WT_manual"
  }

#IS THERE A STATISTICAL DIFFERENCE BETWEEN THE Steady_State phases?
t <- filter(Present_Data, Phase == "Steady_State")

#is there a general difference in distances recorded between the data sets?
SS_Stat_Diff <- t.test(t[which(t$Cell_Line == "WT_manual"),]$Distance,
                             t[which(t$Cell_Line == "WT_auto"),]$Distance)
SS_Stat_Diff$p.value #p < 0 suggests a statistical difference / p > 0 suggests not statistical difference


#if there is a difference in distance, is it statistically less? 
SS_Diff_Less <- t.test(t[which(t$Cell_Line == "WT_manual"),]$Distance,
                             t[which(t$Cell_Line == "WT_auto"),]$Distance, alternative = "less")
SS_Diff_Less$p.value #p = 1 suggests new cell line lower distances than Random 

#if there is a difference in distance, is it statistically Greater?
SS_Diff_Greater <- t.test(t[which(t$Cell_Line == "WT_manual"),]$Distance,
                                t[which(t$Cell_Line == "WT_auto"),]$Distance, alternative = "greater")
SS_Diff_Greater$p.value #p = 1 suggests new cell line has higher distances than Random

#record  Random p values in table
SS_Diff <- SS_Stat_Diff$p.value
SS_Less <- SS_Diff_Less$p.value
SS_Greater <- SS_Diff_Greater$p.value

SS_p_values <- tibble(SS_Diff, SS_Less, SS_Greater) %>%
  transmute(Comaprision = "Steady_State_Data",
            General_Stat_Diff = SS_Diff,
            Diff_Statistically_Less = SS_Less,
            Diff_Statistically_Greater = SS_Greater) 

#says whether there is a stat difference between the new Steady_State data and Random Control
if (SS_Stat_Diff$p.value < 0.05) {
  if (SS_Diff_Less$p.value < SS_Diff_Greater$p.value){
    outcome_2 <- "The WT_auto has statistically higher distances in Steady_State phase than the WT_manual"
  }  else {
    outcome_2 <- "The WT_auto has statistically lower distances in Steady_State phase than the WT_manual"
  }}else {
    outcome_2 <- "There is no statistical difference in distances between the WT_auto in Steady_State phase and the WT_manual"
  }

p_values <- Exp_p_values %>% 
  bind_rows(SS_p_values)


setwd(paste(File_Path, Cell_Type, "Nearest_Neighbour", New_Line_Name, "nascent_Xist_dynamics", sep="\\"))

write_csv(p_values, "Auto_vs_Manual_NNA_t-test_p_values.csv")
